...
// ========================================================
//               5.79in Type B (LUT from OTP)
// Datasheet:
//  - https://files.waveshare.com/wiki/5.79inch-e-Paper-Module/5.79-inch-e-Paper-user-manual.pdf
// This display has 2 controllers: M and S
//   * The register addresses for the M controller range from 0x00 to 0x79
//   * The register addresses for the S controller are the M controller's register addresses + 0x80
...
void WaveshareEPaper5P7In::initialize() {
  ...
  // COMMAND PANEL SETTING (M)
  this->command(0x44);  // Set Ram X- address Start / End position
  this->data(0x00);
  this->data(((this->get_width_controller() / 2 - 1) >> 3) & 0xFF);
  this->command(0x45);  // Set Ram Y- address  Start / End position
  this->data((this->get_height_internal() - 1) & 0xFF);
  this->data(((this->get_height_internal() - 1) >> 8) & 0xFF);
  this->data(0x00);
  ...
  this->command(0x4E);  // X counter
  this->data(0x00);

  this->command(0x4F);  // Y counter
  this->data((this->get_height_internal() - 1) & 0xFF);
  this->data(((this->get_height_internal() - 1) >> 8) & 0xFF);
  this->wait_until_idle_();
  ...
  // COMMAND PANEL SETTING (S)
  this->command(0xC4);  // Set Ram X- address Start / End position
  this->data(((this->get_width_controller() / 2 - 1) >> 3) & 0xFF);
  this->data(0x00);
  this->command(0xC5);  // Set Ram Y- address  Start / End position
  this->data((this->get_height_internal() - 1) & 0xFF);
  this->data(((this->get_height_internal() - 1) >> 8) & 0xFF);
  this->data(0x00);
  this->data(0x00);

  this->command(0xCE);  // X counter
  this->data(((this->get_width_controller() / 2 - 1) >> 3) & 0xFF);

  this->command(0xCF);  // Y counter
  this->data((this->get_height_internal() - 1) & 0xFF);
  this->data(((this->get_height_internal() - 1) >> 8) & 0xFF);
  this->wait_until_idle_();
}
